defmodule DomovoyGitPlugin.Runner.RebaseAbort do
  @moduledoc """
  Aborts an in-progress rebase and returns a snapshot with `operation: :idle`.
  ...
  illustrative.
      ...
      iex> input = DomovoyGitPlugin.Runner.RebaseAbort |> DomovoyCore.Runner.changeset(params) |> Ecto.Changeset.apply_changes()
      iex> context = %DomovoyCore.Context{job: DomovoyCore.Job.new("run"), node: "rebase_abort"}
      iex> DomovoyGitPlugin.Runner.RebaseAbort.run(input, context)
  ...
  def run(%Input{working_directory: directory}, %Context{node: node_name}) do
    with :ok <- Capabilities.rebase_abort(directory, node_name, @directory) do
